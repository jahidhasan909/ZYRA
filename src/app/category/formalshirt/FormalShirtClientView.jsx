...
const FormalShirtClientView = ({ initialformalshirt }) => {
    const [shirts, setShirts] = useState(initialformalshirt);
    const [selectedSort, setSelectedSort] = useState("Date (New > Old)");

 
    const handleSort = (key) => {
        let sortedData = [...shirts];
        ...
        if (key === "low-to-high") {
            ...
            setShirts(initialformalshirt);
            setSelectedSort("Date (New > Old)");
            return;
        }
        setShirts(sortedData);
    };
    ...
    return (
        <main className="px-4 mt-19 md:px-14 py-10">
            ...
            <div className="flex justify-between items-center mb-8 text-xs md:text-sm text-slate-500 font-medium border-b border-slate-100 dark:border-slate-800 pb-4">
                <div className=' uppercase'>MAN / Formal Shirt</div>
                ...
                {shirts.map((item) => (
                    ...
                    >
                        ...
                        <div className="pt-4 pb-2 space-y-2 flex-1 flex flex-col justify-between">
                            ...
                            <div className="pt-2">
                                <Link href={`/category/formalshirt/${item.id}`} className="w-full block">
                                    <Button 
...
export default FormalShirtClientView;